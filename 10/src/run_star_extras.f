...
! ***********************************************************************
      ...
      integer function extras_finish_step(id, id_extra)
         ...
         !f = s% x_integer_ctrl(1)     ! User
         !s% xtra1 = s% xa(h1,s% nz)  ! Central H content

        ! output at log center density = {... 1.0, 1.2, 1.4, 1.6, 1.8, 2.0 ... }
        ! if ((floor(f * s% xtra1_old) - floor(f * s% xtra1) .ne. 0)) then
        ! Output when Central H content = 0.68
        ! Output when Central H content = 0.33
        ! Output when Central H content = 0.01
        h1 = s% net_iso(ih1)
        s% xtra1 = s% xa(h1,s% nz)  ! Central H content
        s% need_to_save_profiles_now = .false.

        if ((s% xtra1 < 0.66) .and.  (s% xtra1_old > 0.66)) then
          ! save a profile & update the history
          s% need_to_update_history_now = .true.
          s% need_to_save_profiles_now = .true.
          write(*,*) 'Saving GYRE model with Xc: ', s% xa(h1,s% nz)
          write(*,*) 'Xc, Xc_old',s% xtra1, s% xtra1_old
        endif

        if ((s% xtra1 < 0.333333) .and.  (s% xtra1_old > 0.333333)) then
          ! save a profile & update the history
          s% need_to_update_history_now = .true.
          s% need_to_save_profiles_now = .true.
          write(*,*) 'Saving GYRE model with Xc: ', s% xa(h1,s% nz)
        ...
        if ((s% xtra1 < 0.01) .and. (s% xtra1_old > 0.01)) then
          ! save a profile & update the history
          s% need_to_update_history_now = .true.
          s% need_to_save_profiles_now = .true.
          write(*,*) 'Saving GYRE model with Xc: ', s% xa(h1,s% nz)
